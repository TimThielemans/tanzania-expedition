...
-- Voer dit uit NA schema.sql, upgrade.sql, upgrade2.sql en upgrade3.sql.
-- ============================================================
...
-- locatie-instellingen (niet overschrijven als ze al bestaan)
insert into public.game_settings (key, value) values
...
insert into public.zones (name, description, tagline, icon, order_index, unlock_type, unlock_password, automatic_unlock)
values
  ('MTB Adventure', 'Trap je een weg door de heuvels van Arusha.', 'Niet iedereen die dwaalt is verdwaald.', '🚵', 1, 'open',     null,         false),
  ('Mt Meru',       'De klim naar 4.562 meter. Adem diep in.',     'Samen naar de top.',                    '⛰️', 2, 'password', 'MERU2026',   false),
  ('Safari',        'Big Five spotten in de Serengeti.',           'Ogen open voor avontuur.',              '🦁', 3, 'password', 'SERENGETI',  false),
  ('Zanzibar',      'Wit zand, blauw water, welverdiende rust.',   'De finish is een paradijs.',            '🌴', 4, 'open',     null,         false)
on conflict do nothing;

-- ---------- challenges ----------
-- creativity_bonus_points > 0 → de reisleider kan ⭐ Uitstekend geven (extra creativiteitspunten).
...
-- ---------- bonusopdracht (niet aan een zone gekoppeld) ----------
insert into public.challenges
...
insert into public.location_events
  (name, description, latitude, longitude, radius_meters, trigger_mode, trigger_type,
   notification_message, challenge_title, challenge_description, challenge_type, points, active)
values
  ('Bibliotheek-checkpoint',
   'Eerste team dat de bibliotheek bereikt krijgt een exclusieve opdracht.',
   -3.366700, 36.680000, 60, 'first', 'location_challenge',
   null,
   'Torens tellen',
   'Hoeveel torens zie je vanaf dit punt? Geef een getal.',
   'text_answer', 20, false),
  ('Marktplein',
   'Elk team dat het marktplein bereikt maakt een teamfoto.',
   -3.370000, 36.690000, 75, 'every', 'location_challenge',
   null,
   'Teamfoto op de markt',
   'Maak hier een foto van het volledige team.',
   'photo_upload', 15, false),
  ('Aankomst finale',
   'Melding aan de reisleider zodra een team de finale bereikt.',
   -3.386925, 36.682995, 100, 'every', 'admin_notification',
   'Een team is aangekomen op de finalelocatie.',
   null, null, 'text_answer', 0, false)
on conflict do nothing;
